...
当前治理推进已完成 `SEC-003E` 预算填报 API 授权接入，安全基线采用“角色 + Entity 数据范围 + 流程责任人”的模型，避免过早建设复杂多维权限矩阵。SEC-003 已保护安全管理 API 与预算查询读取 API，SEC-003B 已保护元数据读写 API，SEC-003C 已保护预算模型读写 API，SEC-003D 已保护预算模板读写 API，SEC-003E 已保护预算填报读写和流程动作 API；生产登录、JWT/SSO、前端安全体验和更多业务写接口授权仍需后续阶段推进。
